...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        Node<T> grandparent = parent.up; //keep a reference of grandparent
        Node<T> ogParent = parent; //keep a reference original parent
        if (child == parent.down[1]) { //rotate left

            parent.down[1] = child.down[0]; //give child's left child to the parents right child
            if (child.down[0] != null) {
                child.down[0].up = parent;
            }

            if (grandparent!=null){
                if (grandparent.down[0]== parent) {  //check if the parent was a right or left child
                ...
                root = child; // if grandparent is null, root is child
            }

            child.down[0]=parent;
            parent.up=child;
        }
        else if (child == parent.down[0]){ //rotate right
            parent.down[0] = child.down[1]; //give child's left child to the parents right child
            if (child.down[1] != null) {
                child.down[1].up = parent;
            }

            if (grandparent!=null){
                if (grandparent.down[1]== parent) {  //check if the parent was a right or left child
                    grandparent.down[1]=child;
                } else {
                    grandparent.down[0]=child;
                }
    ...
    /**
     * Test case 1 if rotating right works (5 nodes)
    ...
    public static boolean test1() {
        ...
        // find nodes for four and five
        Node<Integer> five = tree.root;
        Node<Integer> four = five.down[0];
        Node<Integer> three = four.down[0];
        Node<Integer> two = three.down[0];
        Node<Integer> one = two.down[0];
        // then rotate node with four and five
        tree.rotate(four, five);

        if (!tree.toLevelOrderString().equals("[ 4, 3, 5, 2, 1 ]")) {
    ...
    /**
     * Test case 2 if rotating left works (9 nodes)
     * @return true if test passes, false otherwise
     */
    public static boolean test2() {
        ...
        //find nodes
        Node<Integer> node1 = tree.root;
        Node<Integer> node2 = node1.down[0];
        Node<Integer> node3 = node2.down[1];
        //rotate nodes
        tree.rotate(node3, node2);
        if (!tree.toLevelOrderString().equals("[ 7, 6, 9, 5, 10, 4, 2, 1, 3 ]")) {
            return false;
        }
        return true;
    }
    /**
     * Test case 3 if exception is thrown with illegal argument
     * @return true if test passes, false otherwise
     */
